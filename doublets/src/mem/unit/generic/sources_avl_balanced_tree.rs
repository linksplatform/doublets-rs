...
/// AVL balanced tree implementation for managing sources of links
pub struct LinksSourcesAvlBalancedTree<T: LinkType> {
...
impl<T: LinkType> LinksSourcesAvlBalancedTree<T> {
    pub fn new(
...
impl<T: LinkType> LinkAvlBalancedTreeBaseAbstract<T> for LinksSourcesAvlBalancedTree<T> {
...
impl<T: LinkType> LinksTree<T> for LinksSourcesAvlBalancedTree<T> {
    fn get_tree_root(&self) -> T {
    ...
    fn clear_node(&mut self, node: T) {
        ...
        link.balance_as_source = T::funty_zero();
    }
...
impl<T: LinkType> AvlTree<T> for LinksSourcesAvlBalancedTree<T> {
    fn get_balance(&self, node: T) -> T {
        if node == T::funty_zero() {
            T::funty_zero()
        } else {
            self.get_link(node).balance_as_source
        }
    }

    fn set_balance(&mut self, node: T, balance: T) {
        if node != T::funty_zero() {
            self.get_mut_link(node).balance_as_source = balance;
        }
    }
